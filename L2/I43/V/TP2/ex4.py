...
def chiffre(clair, k):
    ...
    while(i < len(clair)):
        chiffre += doci[(dico[clair[i]]*k[0] + k[1])%29]
        i += 1
...
def cryptanalyse(fic):
    ...
    k += [egcd(k[0],29)[1]]
    clair = (dechiffre(cryptogramme,k,29))
    return k,clair

#il suffit de rechiffrer le texte avec la cle qu'on a trouve en avec la resolution affine
clair = cryptanalyse("crypto6.txt")
k = clair[0]
clair = clair[1]
print(chiffre(clair,k))
