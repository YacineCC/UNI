...
def dechiffre(cryptogramme,k,n):
    #modifications de la fonction pour pouvoir inclure les caracteres , ' .
...
#fonction qui transforme le contenu d'un fichier en chaine de caractere
def fic2str(fic):
...
def brute_force(fic,n,eps):
    ...
    dico['y'] = 0.28
    dico['z'] = 0.15
    cryptogramme = fic2str(fic)
    big = enumere_keys(29)
    for waa in big:
        waa[2] = egcd(waa[0],n)[1]
        test = dechiffre(cryptogramme,waa,n)
...
def cryptanalyse(fic):
    ...
    cryptogramme = fic2str(fic)
    tab = analyse_freq(cryptogramme)
    #creation du bigrame avec la concatenation des deux lettres succesives les plus frequentes
    bigramme = tab[0] + tab[3]
    #creation de la cle
    k = resolution_affine(bigramme)
    k += [egcd(k[0],29)[1]]
    print(dechiffre(cryptogramme,k,29))
    return k
